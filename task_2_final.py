...
def main():
    ...
    # Accessing the local regtest node and importing the p2sh_address
    proxy = NodeProxy("test", "test").get_proxy() # use ur own user and password.
    ...
    # Transaction detail with the specific UTXO (for the p2sh_address). Used that for testing.
    # utxos = [{'txid': 'd39be75058c0b23f736d84a597f9ef970debbb99640d4c0f7e71bd07fdc7580d', 'vout': 1, 'amount': 3.0}]
    
    # All transactions from the local regtest node
    utxos = proxy.listenunspent(0, 99999999, [p2sh_address])
